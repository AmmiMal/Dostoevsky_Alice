...
class QuizScene(Scene):
    def __init__(self, resource: dict, handlers):
        super().__init__(resource, handlers)
        ...
        if self.current_line < len(self.replics):
            ...
            if self.current_line == len(self.replics):
                ...
                answer = {
                    ...
                    "buttons": replica["buttons"]
                }

                if self.last_answer_result is not None:
                    result_text = "Правильно!" if self.last_answer_result == 'correct' else "Неверно"
                    self.last_answer_result = None
